...
namespace Lan_Messenger
{
	...
	public class FormMessage : System.Windows.Forms.Form
	{
		...
		internal void AddText(string person,string message)
		{
            ...
            if (message.IndexOf("Agreed webcam chat") != -1)
            {
            ...
            {
                this.Show();
                switch (MessageBox.Show(Global.server.GetfullName(contact) + " would like to invite you to chat webcam. do you agree?", "Chat Webcam...", MessageBoxButtons.YesNo))
                {
                    case DialogResult.Yes:
                        IPHostEntry IP = Dns.GetHostByName(Dns.GetHostName().ToString());
                        string tempIP = IP.AddressList[0].ToString(); // IP máy phản hồi
                        Global.server.Send(Global.username, contact, "Agreed webcam chat - IP: " + tempIP);
                        ...
                        break;
                    case DialogResult.No:
                        Global.server.Send(Global.username, contact, Global.username + " do not want to chat Webcam!");
                        this.DialogResult = DialogResult.None;
                        break;
                }
            }
        ...
        private void picWebcam_Click(object sender, EventArgs e)
        {
            ...
            if (Global.server.IsVisible(contact))
            {
                ...
                Global.server.Send(Global.username, contact, " Asking to share webcam chat - IP: " + IP_rq);
            }
            else
            {
                MessageBox.Show("This person does not have Online. Please try again later! ", " Error!");
            }
        }
        ...
        public void ServerStart()
        {
            ...
            Global.server.Send(Global.username, contact, " Accepted file submission request (IP: " + IP_temp + ")"); // Thông báo tới người gửi
